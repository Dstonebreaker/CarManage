    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            tree = $('#gridList').datagrid({
                title: '车辆信息',
                url: sys.contextPath + '/car/insurance!doNotNeedSecurity_getcarList.cxl',
                queryParams: {
                ...
                columns: [[
                    {
                        ...
                        formatter: function (value, row) {
                            var result = sys.formatString('<a href="javascript:void(0)" class="btn00" onclick="renewalFun(\'{0}\',\'{1}\');">续保</a>', row.carId, row.carNo);
                            return result;
                        }
                    }
            ...
            grid = $('#grid').datagrid(
                    {
                        title: '',
                        url: sys.contextPath + '/car/vinsurance!grids.cxl',
                        queryParams: {
                            "QUERY_t#dictIdFlag_S_EQ": '<%=WebMsgUtil.YOUXIAO%>'
                        },
                        striped: true,
                        ...
                        columns: [[
                            ...
                            },
                            {
                                ...
                                formatter: function (value, row) {
                                    ...
                                    str += sys.formatString('<a href="javascript:void(0)" class="btn2" onclick="editFun(\'{0}\');">编辑</a>', row.insuId);
                                    <%}%>
                                    <%if (securityUtil.havePermission("/car/insurance!delete")) {%>
                                    str += sys.formatString('<a href="javascript:void(0)" class="btn3" onclick="removeFun(\'{0}\');">删除</a>', row.insuId);
                                    <%}%>
                                    return str;
                                }
                            }]],
        ...
        var renewalFun = function (carId, carNo) {
            ...
            var dialog = parent.sys.modalDialog({
                title: '续费保险信息',
                url: sys.contextPath
                + '/securityJsp/carinsurance/car_form.jsp?carId=' + carId + "&carNo=" + carNo_new,
                buttons: [{
                    text: '续保',
                    handler: function () {
                        dialog.find('iframe').get(0).contentWindow.submitForm(
...
<div data-options="region:'west'" style="width:240px">
    <div id="westtoolbar" style="display: none;">
        <table>
            <tr>
                <td><input id="input_value" class="easyui-textbox"  style="width:120px"/></td>
