...
<html>
  ...
  <body style="margin:0px;padding:0px">
	...
	<form method="post" id="searchForm">
		...
		车型：<input id="dictIdCarType" name="QUERY_t#dictIdCarType_S_EQ">
	...
	var approval = function(id) {
		var dialog = parent.sys.modalDialogNew({
			title : '审批',
			href : sys.contextPath + '/securityJsp/apply/car_apply_approval_form.jsp?id=' + id,
			width: 750,
			height: 500,
			...
			buttons : [ {
				...
				handler : function() {
					...
					var dataform = dialog.find("#detailForm");
		    		if (dataform.form('validate')) {
		    			parent.$.messager.progress({
		    				text : '数据处理中....'
		    			});
						var params = dialog.find("#applyId").serialize();
						params += "&apply.applyCheck2=" + dialog.find("#applyCheck").textbox('getText');
