...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sys.modalDialog({
			title : '添加数据字典',
			url : sys.contextPath + '/securityJsp/maintain/dicDataForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var editFun = function(id) {
		var dialog = parent.sys.modalDialog({
			title : '修改数据字典',
			url : sys.contextPath + '/securityJsp/maintain/dicDataForm.jsp?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(
	...
	$(function() {
		...
		grid = $('#grid')
				.datagrid(
						{
							queryParams : {'QUERY_t#sysDictionaryClass#dictClasIsSelf_I_EQ' : '1'},
							title : '',
